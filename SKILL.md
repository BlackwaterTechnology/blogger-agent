...
### 图片生成（Text-to-Diagram）

为保证 Agent 零外部依赖，图片生成放弃了繁重的本地子进程模式，全面集成 Text-to-Diagram 能力：
...
### Step 2: 插图生成（Text-to-Diagram）

如果文章需要插图或封面，不要依赖外部图片模型，请利用你优秀的代码能力编写**结构图 (PlantUML) 或流程图 (Mermaid)**。
...
如果是在 MCP 模式下，请直接将代码传递给 `generate_diagram` tool，将 `output_path` 指向 payload 目录。
