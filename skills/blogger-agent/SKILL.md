...
把任意输入（草稿、对话、观点、技术笔记）转化为**结构清晰、论点锐利、以图代言**的中文技术文章，生成插图与封面，调用本地 `blogger` CLI 推送到主流博客平台。

与浏览器自动化不同，此 skill 走的是「Markdown payload + CLI」管道。本 skill 同时是**创作助手 + 编辑助手 + 建模助手**：它会逼你先想清楚「主张是什么」「证据是什么」「哪些概念该建模成图」再开写；写完后强制做一次反 AI 腔自审。

**核心原则：模型 / 图表 / 数据图 > 文字描述。** 一张架构图、状态机、对比雷达、流程图能解释清楚的事，不要写成 200 字段落让读者自己脑补。技术写作里能图示化的对象（架构、流程、调用链、状态变化、分类层级、数据分布、维度对比）就建模成图——文字只用来补图无法说清的细节、上下文、推理。自动发布管线对正文配图（无数量上限、自动上传、自动加图注）已经成熟，**多画图、画好图**是这一版的最大杠杆。

所有发到微信公众号的文章会自动把"创作声明"设为"个人观点，仅供参考"。
...
- **数量原则**：每个 ### 段落原则上对应一个建模对象（架构 / 流程 / 对比 / 状态 / 数据），段落讲不清楚的就建模一张图。"图越多越花哨"不是目的，**"用图代替能用图代替的文字"才是**。重复服务同一论点的图删掉。
- 图片放进 Payload 目录（如 `articles/<文章标题>/`）。源码（`.mmd` / `.puml` / `.py`）也一并保留，便于回溯与迭代修图。

#### 2.2 工具选择（建模思路 → 工具）

把阶段 1A Q4 勾的每一项映射到下表的工具。**默认假设：每段技术解释先尝试建模成图**——只有图都画过了仍不到位，才退回到纯文字。
...
1. **横纵比窗口**：
   - **正文插图**：横向 `1:1 ~ 2:1`（最长不建议超过 `2.5:1`），或竖向 `1:1 ~ 3:4`。绝不出现接近 `1:3` 或 `6:1` 的极端比例。
   - **横向对比布局（避坑建议）**：虽然 `A -[hidden]right-> B` 能实现并排，但如果节点内部是单线流动，容易产出极扁的“长条图”。**推荐使用“纵向堆叠的横向卡片”模式**：使用两个 `rectangle` 垂直堆叠，内部节点使用 `-right->` 横向流动。这种布局在手机端阅读体验极佳，且比例稳定（通常在 `2:1` 左右）。
...
> 💡 **图位提示**：每种骨架内嵌的 image 占位是**最低数量**，按阶段 1A Q4 视觉建模清单决定额外加几张。理想形态：每个 ### 段落至少配一张图服务它的论点（架构 / 流程 / 状态 / 对比 / 数据 / 时间线择一）。骨架里没标 image 不代表那段不放图——只是没硬性要求。
...
- **整篇文章只配 1 张封面 / 0 张正文图**：技术文章里大段文字描述能图示化的概念（架构、流程、对比、状态机、分类层级），是最大的反模式。先按阶段 1A Q4 列出 2-4 个建模对象，每个对象配 1 张图——这是这一版的最大杠杆。
- **跳过阶段 1A Q4 视觉建模清单**：产出会回到"全文文字 + 1 张装饰性 AI 插图"的低质量形态。Q4 是把"建模优先"落到位的钩子，必填。
- **用 200 字段落罗列「3 个层次 / 5 个阶段 / 4 种角色」却不画图**：这种结构信息天生适合 mindmap / `@startwbs` / flowchart，文字版读者扫完就忘。规则：**只要正文里出现"3 个 / 5 个 / 几类"这种枚举词，先停下来想能不能换成一张图**。
- **把对比写成大段文字而不是表格 + 雷达图**：维度对比、产品对比、方案对比统一走"Markdown 表格 + matplotlib 雷达图"组合，读者一眼能看完。光写"A 比 B 更适合 X 场景"这种文字结论，等于浪费对比这件事最大的认知红利。
