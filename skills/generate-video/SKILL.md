name: generate-video
description: Multi-provider video generation skill. Create cinematic videos, explainers, and summaries using various AI platforms. Currently supports Google NotebookLM (programmatic access) with planned support for Hyperframes and others. Activates on video generation requests or explicit /generate-video.

# Video Generation Skill

This skill orchestrates video creation across multiple AI providers.
...
- **Metadata & Publishing**: Every video needs a summary or introduction for publishing platforms (WeChat, CSDN, etc.). Use the provider's AI (e.g., `notebooklm ask "为该视频写一段摘要和发布简介"` or `notebooklm summary`) to generate these. Save the results in `metadata.txt` or `README.md` within the video's specific subdirectory.

## Providers

### 1. Google NotebookLM (Current Primary)
Complete programmatic access to Google NotebookLM. Create notebooks, add sources, and generate explainers or cinematic videos based on your documents.

#### Installation
```bash
pip install notebooklm-py
```
*Follow the installation guide in the NotebookLM section below for detailed setup.*

### 2. Hyperframes (Planned)
*Integration pending. Planned for high-quality synthetic video generation.*

---

# NotebookLM Provider Guide
...
$ notebooklm ask "What is X?" --json
{
  ...
  "is_follow_up": false,
  "references": [
    {"source_id": "abc123...", "citation_number": 1, "cited_text": "Relevant passage from source..."},
...
4. Spawn a background agent using Task tool:
   ```python
   Task(
     prompt="Wait for artifact {artifact_id} in notebook {notebook_id} to complete, then download. Use: notebooklm artifact wait {artifact_id} -n {notebook_id} --timeout 600 Then: notebooklm download audio ./podcast.mp3 -a {artifact_id} -n {notebook_id}",
...
2. Spawn a background agent to wait for all sources:
   ```python
   Task(
     prompt="Wait for sources {source_ids} in notebook {notebook_id} to be ready. For each: notebooklm source wait {id} -n {notebook_id} --timeout 120 Report when all ready or if any fail.",
     subagent_type="general-purpose"
   )
   ```
...
3. Spawn a background agent to wait and import:
   ```python
   Task(
     prompt="Wait for research in notebook {notebook_id} to complete and import sources. Use: notebooklm research wait -n {notebook_id} --import-all --timeout 300 Report how many sources were imported.",
     subagent_type="general-purpose"
   )
   ```
