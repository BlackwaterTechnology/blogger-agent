...
```
 ...
 · 3:4比例与1200px宽度(10)· 4-8字爆破冲突Hook (10)         · 350-700字硬约束(7)     · 标题≤20字与0破折号(8)
 · 字号≥28px底线无溢出(10)· 3行高对比度微型认知卡 (8)       · 换行独立与0粘连(7)     · 结尾大号互动CTA (6)
 · 模板流转完整度 (8)    · 0 AI俗套(蓝脑/机械手/乱码)(7)    · 0 Markdown与0#标签(6)   · 合集符合blogger.toml(6)
 · 统一杂志配色 (7)
```
...
6. 0 LaTeX 行内公式与原生符号校验：
   - 检查正文和卡片中是否存在 `$\rightarrow$`、`$\Leftarrow$` 等 LaTeX 语法。
   - 一律自动替换为原生 Unicode 符号（`→`、`⇒`、`⚡`、`💡`、`💬`、`❌`、`✅`）。

7. 微信合集校验 (collection)：
   - `collection` 必须且只能从 `blogger.toml` 的 `photo_collections` 列表中选择。
   - 若不合法，自动修正为语义最贴合的合法合集。

8. 封面爆破 Hook 审查 (Cover Hook Audit)：
